...
public:
    ...
    hashdata UserToAdd(string a, int num);
    void SearchResult(string a)const;
private:
    static const int SIZE = 60;
    List<hashdata> Table[SIZE];
};
...
template<class hashdata>
hashdata HashTable_I<hashdata>::UserToAdd(string a, int num)
{
	int index=hash(a);
	Table[index].startIterator();
	for(int i=1;i<num; i++)
...
template<class hashdata>
void HashTable_I<hashdata>::SearchResult(string a)const
{
	int index=hash(a);

	countBucket(index);

	cout <<endl<<" This's a list of users with interest as searched"<<endl<<endl;

	printBucket(cout,index);
}
